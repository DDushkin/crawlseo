...
import { parseCompetitorGap } from "@/lib/dataforseo/client";
...
async function ownedSite(siteId: string, userId: string) {
  return db.site.findFirst({ where: { id: siteId, userId }, select: { id: true, domain: true, gscProperty: true } });
}
...
export async function POST(req: Request, { params }: { params: Promise<{ siteId: string }> }) {
  ...
  const content = buildContentBrief({
    ...
    query: target.query, pageUrl: target.page?.url ?? null, gsc, competitorResults,
  });
