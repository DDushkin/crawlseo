...
export default async function PageInventoryPage({ params }: { params: Promise<{ siteId: string }> }) {
  ...
  return <div>
    ...
    <div className="panel overflow-x-auto"><table className="w-full min-w-[640px] text-sm"><thead><tr className="border-b border-border text-left text-xs uppercase text-muted-foreground"><th className="p-4">Page</th><th className="p-4">Type</th><th className="p-4">State</th><th className="p-4 text-right">Targets</th><th className="p-4 text-right">Actions</th></tr></thead><tbody className="divide-y divide-border/50">{pages.map((page) => <tr key={page.id}><td className="p-4"><a href={page.url} target="_blank" rel="noopener noreferrer" className="break-all text-signal hover:underline">{page.url}</a></td><td className="p-4">{page.pageType || "—"}</td><td className="p-4">{page.state}</td><td className="p-4 text-right">{page._count.keywordTargets}</td><td className="p-4 text-right">{page._count.actions}</td></tr>)}</tbody></table>{pages.length === 0 && <p className="p-5 text-sm text-muted-foreground">No managed pages yet. Sync GSC, run a complete crawl, import a page map, or add a canonical page to start a keyword map.</p>}</div>
