...
export default async function AiVisibilityPage({ params }: { params: Promise<{ siteId: string }> }) {
  ...
  return <div><PageHeader eyebrow={site.domain} title="AI visibility" description="Three distinct measurements: Google's AI search impressions, identifiable GA4 AI referrals, and sampled ChatGPT citations. No opaque blended score." />
    <div className="mb-5 grid gap-3 md:grid-cols-3">
      <div className="panel p-4"><p className="text-xs uppercase tracking-wide text-muted-foreground">Google AI impressions</p><p className="mt-2 text-2xl font-semibold">{googleDays.length ? googleDays.reduce((sum, day) => sum + day.impressions, 0).toLocaleString() : "Unavailable"}</p><p className="mt-1 text-xs text-muted-foreground">GSC export · {googleDays.length} imported day(s) in the recent 28-day window · {imports[0] ? `latest import ${imports[0].importedAt.toISOString().slice(0, 10)}` : "no export"}</p></div>
      <div className="panel p-4"><p className="text-xs uppercase tracking-wide text-muted-foreground">AI-referred sessions</p><p className="mt-2 text-2xl font-semibold">{site.lastGa4SyncAt ? referralDays.reduce((sum, day) => sum + day.sessions, 0).toLocaleString() : "Unavailable"}</p><p className="mt-1 text-xs text-muted-foreground">GA4 known AI referrers · {site.lastGa4SyncAt ? `${referralDays.reduce((sum, day) => sum + day.keyEvents, 0)} key events` : "connect and sync GA4"}</p></div>
    ...
    <div className="mt-5 grid gap-4 lg:grid-cols-2"><section className="panel p-5"><h2 className="font-heading text-lg font-semibold">Recent Google AI imports</h2>{imports.length ? <ul className="mt-3 space-y-2 text-sm">{imports.map((item) => <li key={item.id}>{item.importedAt.toISOString().slice(0, 10)} · {item.firstDate.toISOString().slice(0, 10)} to {item.lastDate.toISOString().slice(0, 10)} · {item.rowCount} days · {item.fileName}</li>)}</ul> : <p className="mt-2 text-sm text-muted-foreground">No export imported. The report may be unavailable if Google has insufficient impressions for this property.</p>}<a className="mt-3 inline-block text-xs text-signal hover:underline" href="https://support.google.com/webmasters/answer/16984139?hl=en" target="_blank" rel="noopener noreferrer">Google report documentation</a></section>
      <section className="panel p-5"><h2 className="font-heading text-lg font-semibold">GA4 organic context</h2><p className="mt-3 text-2xl font-semibold">{site.lastGa4SyncAt ? organicDays.reduce((sum, day) => sum + day.sessions, 0).toLocaleString() : "Unavailable"}</p><p className="text-xs text-muted-foreground">Organic Search sessions · {site.lastGa4SyncAt ? `${organicDays.reduce((sum, day) => sum + day.keyEvents, 0)} key events` : "no successful GA4 sync"}</p><p className="mt-3 text-xs text-muted-foreground">This is a separate GA4 outcome metric, not a Search Console click count.</p></section></div>
