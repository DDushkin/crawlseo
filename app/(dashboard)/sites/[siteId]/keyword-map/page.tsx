...
import { CreateBriefButton, KeywordTargetForm, SerpResearchButton } from "@/components/operator/workspace-forms";
...
export default async function KeywordMapPage({ params }: { params: Promise<{ siteId: string }> }) {
  ...
  const [pages, targets, market] = await Promise.all([
    ...
    db.dataForSeoSettings.findUnique({ where: { siteId }, select: { locationCode: true, languageCode: true } }),
  ]);
  return <div>
    ...
    <KeywordTargetForm siteId={siteId} pages={pages} country={targets[0]?.country || (market && countryForDataForSeoLocation(market.locationCode)) || "US"} language={targets[0]?.language || market?.languageCode || "en"} />
    ...
    <div className="panel overflow-x-auto"><table className="w-full min-w-[760px] text-sm"><thead><tr className="border-b border-border text-left text-xs uppercase text-muted-foreground"><th className="p-4">Query</th><th className="p-4">Decision</th><th className="p-4">Target</th><th className="p-4">Market</th><th className="p-4">Briefs</th><th className="p-4">Next</th></tr></thead><tbody className="divide-y divide-border/50">{targets.map((target) => <tr key={target.id}><td className="p-4 font-medium">{target.query}</td><td className="p-4">{target.decision.replaceAll("_", " ").toLowerCase()}</td><td className="max-w-xs break-all p-4">{target.page?.url || "—"}</td><td className="p-4">{target.country}/{target.language}</td><td className="p-4">{target._count.briefs}</td><td className="p-4"><div className="space-y-2"><SerpResearchButton siteId={siteId} targetId={target.id} /><CreateBriefButton siteId={siteId} targetId={target.id} /></div></td></tr>)}</tbody></table>{targets.length === 0 && <p className="p-5 text-sm text-muted-foreground">No confirmed targets. Start with a query from GSC or a customer question.</p>}</div>
