...
1. Back up the production PostgreSQL database and record the currently running image/commit. Confirm the application's `DATABASE_URL`, `NEXTAUTH_SECRET`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `NEXTAUTH_URL`, `AUTH_TRUST_HOST=true`, and `CRON_SECRET` are present. Keep the existing encryption secret unchanged; changing it would make stored DataForSEO and GA4 keys unreadable.
...
5. Leave DataForSEO in Sandbox until the account key and site market are confirmed. Sandbox responses are synthetic. Switching a site to Live and confirming a cost preview is the only way the research/panel workflows can spend its DataForSEO credit. The pilot guard allows US$0.15 of recorded spend per site and reserves US$0.06 before each in-flight request. DataForSEO reports the actual charge afterward, so this is **not a guaranteed provider-side spending cap**; set a limit with the provider if a hard account ceiling is required.

No new required environment variable was added. GA4 no longer requests an Analytics scope through the Google sign-in client. For GA4, create a service account in a **separate Google Cloud project**, enable the **Google Analytics Data API** there, and create a JSON key. In Google Analytics property access management, add the service-account email with **Viewer** role. Upload the key in CrawlSEO site Settings; it is encrypted server-side, shared across this owner's sites, and never displayed after upload. Then enter each site's numeric GA4 property ID under AI Visibility and run a sync. Connect checks the service account's property access before saving. The GA4 property must correspond to the selected site; this is an owner selection, not automatically proven from its domain. Removing the credential stops future syncs but retains historical rows. Do not put the JSON key in Coolify environment variables or the repository.
...
The AI panel is deliberately resumable. Confirming a preview saves the exact question snapshot and estimated cost (not a guaranteed maximum); it does not call DataForSEO inside that browser request. The user can process the next question manually, and the cron worker can continue the same queue after a browser closes. A three-minute lease prevents overlapping workers. Cancel between questions to stop remaining requests. A request already in flight can still incur a charge. No cron endpoint accepts a caller-provided site ID.
...
3. For a service page or article, assign one strategic target in Keyword Map. If the SERP will change the brief, use **Research SERP** and confirm its cost preview, then create a Content Brief. The brief snapshots the observed organic results, questions, related searches, GSC data, and crawl facts available for that target; missing evidence stays unavailable. Edit the copywriter plan, mark it ready, and have the copywriter implement it on Strum. Crawl the site afterward. CrawlSEO does not publish content automatically.
